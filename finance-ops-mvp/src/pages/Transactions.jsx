...
export default function Transactions(){
  ...
  const list = st.transactions.filter(t => {
    return !q || t.code.toLowerCase().includes(q.toLowerCase()) || (t.description||'').toLowerCase().includes(q.toLowerCase())
  })
  const me = st.me
    ...
    <div className="grid">
      ...
      <div className="card">
        <table className="table">
          <thead>
            <tr><th>Code</th><th>Type</th><th>Amount</th><th>Status</th><th>Creator</th><th>Created</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {list.map(t=>(
              <tr key={t.id} className={`status-${t.status}`}>
                ...
                <td>
                  {me?.role==='employee' && t.status==='draft' && (
                    <button className="btn" onClick={()=>submitTransaction(t.id)}>Submit</button>
                  )}
                </td>
              </tr>
            ))}
