...
const LS_KEY = 'finance_ops_store_v1'
...
const defaultState = {
  ...
  transactions: [
    {
      id: 1001, code: 'TX-2025-00001', type: 'revenue', amount: 12000, currency: 'SAR', description: 'August sales batch A',
      status: 'submitted', created_by: 1,
      created_at: dayjs().subtract(2, 'day').toISOString(),
      submitted_at: dayjs().subtract(2, 'day').add(30, 'minute').toISOString(),
      pre_approved_at: null, final_approved_at: null, disbursed_at: null,
      notes: [], attachments: []
    },
    {
      id: 1002, code: 'TX-2025-00002', type: 'expense', amount: 3250, currency: 'SAR', description: 'Office supplies',
      status: 'pre_approved', created_by: 1,
      created_at: dayjs().subtract(1, 'day').toISOString(),
      submitted_at: dayjs().subtract(1, 'day').add(20, 'minute').toISOString(),
      pre_approved_at: dayjs().subtract(1, 'day').add(2, 'hour').toISOString(),
      final_approved_at: null, disbursed_at: null,
      notes: [{ author_id: 2, text: 'Attach invoice, please', created_at: dayjs().subtract(1, 'day').add(30, 'minute').toISOString() }],
      attachments: []
    }
  ],
  audit: []
...
export function requireRole(role){
  const me = state.me
  if(!me || (role && me.role !== role)) throw new Error('Unauthorized')
  return me
}
...
export function createTransaction(data){
  ...
  const tx = {
    id, code, status: 'draft', currency: 'SAR', created_by: me.id,
    created_at: dayjs().toISOString(), updated_at: dayjs().toISOString(),
    submitted_at: null, pre_approved_at: null, final_approved_at: null, disbursed_at: null,
    notes: [], attachments: [], ...data
  }
  setState(s => ({...s, transactions: [tx, ...s.transactions]}))
  log('transaction', id, 'create', {}, tx)
  return tx
}
...
export function updateTransaction(id, patch){
  ...
  const next = { ...old, ...patch, updated_at: dayjs().toISOString() }
  state.transactions[idx] = next
  setState(s => ({...s}))
  log('transaction', id, 'update', old, next)
  return next
}
...
export function submitTransaction(id){
  ...
  if(tx.status!=='draft') throw new Error('Only draft can be submitted')
  updateTransaction(id, { status: 'submitted', submitted_at: dayjs().toISOString() })
  log('transaction', id, 'submit', {}, {})
}

export function preApprove(id){
  const me = requireRole('supervisor')
  const tx = state.transactions.find(t=>t.id===id)
  if(!tx || !['submitted','returned'].includes(tx.status)) throw new Error('Not in supervisor queue')
  updateTransaction(id, { status: 'pre_approved', pre_approved_at: dayjs().toISOString() })
  log('transaction', id, 'pre_approve', {}, { actor: me.id })
}
...
export function finalApprove(id){
  ...
  updateTransaction(id, { status: 'final_approved', final_approved_at: dayjs().toISOString() })
  log('transaction', id, 'final_approve', {}, { actor: me.id })
}
...
export function rejectTx(id){
  ...
  log('transaction', id, 'reject', {}, { actor: me.id })
}
...
export function returnTx(id, note){
  ...
  updateTransaction(id, { status: 'returned' })
  log('transaction', id, 'return', {}, { actor: me?.id, note })
}
...
export function disburse(id){
  ...
  updateTransaction(id, { status: 'disbursed', disbursed_at: dayjs().toISOString() })
  log('transaction', id, 'disburse', {}, { actor: me.id })
}
...
export function addNote(id, text){
  ...
  updateTransaction(id, { notes: tx.notes })
  log('transaction', id, 'note', {}, { actor: me?.id })
}

export function kpis(fromISO, toISO){
  const list = state.transactions.filter(t=>{
    const ts = dayjs(t.created_at)
    return (!fromISO || ts.isAfter(dayjs(fromISO))) && (!toISO || ts.isBefore(dayjs(toISO)))
  })
  ...
  const pending = state.transactions.filter(t=>['submitted','pre_approved'].includes(t.status)).length
  return { revenue, expense, net, pending }
}

export function monthlySeries(){
  // simple last 6 months mock
